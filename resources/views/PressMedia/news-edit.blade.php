<div id="ownerTable">
    <table id="ownerInfo" class="table table-bordered table-hover"
        ...
        $i = 1;
        $inc = 0;
         $news = json_decode($item->news);

        ?>
        @foreach($news as $key => $value)
        <tr id="ownerInfoRow{{$inc}}">
            <td class="text-center">
                {!! Form::text("headline[$key]",$value->title,['class' => ' form-control input-md required','placeholder' => '']) !!}
                {!! $errors->first('link','<span class="help-block">:message</span>') !!}
            </td>
            <td class="text-center">
                {!! Form::text("link[$key]",$value->link,['class' => ' form-control input-md required','placeholder' => '']) !!}
                {!! $errors->first('link','<span class="help-block">:message</span>') !!}
            </td>
